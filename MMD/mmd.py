...
from MMD.cuda_backend import compute_sig_kernel_batch_varpar_from_increments_cuda, \
...
class SigKernel():
    ...
    def compute_HigherOrder_Gram(self, K_XX, K_XY, K_YY, lambda_, sym=False):
        ...
        """
        return _SigKernelHigerOrderGram.apply(K_XX, K_XY, K_YY, self.static_kernel_higher_order,
                                              self.dyadic_order_higher_order, lambda_, sym, self._naive_solver)
    ...
    def _compute_higher_order_mmd(self, X, Y, lambda_, estimator='b'):
        ...
        K_XY_1 = self.compute_Gram(X, Y, sym=False,
                                   return_sol_grid=True)  # shape (batch_X, batch_Y, length_X, length_Y)
        end_gram = time.time()
        # print(end_gram-start_gram)
...
class _SigKernelGram(torch.autograd.Function):
    ...
    def forward(ctx, X, Y, static_kernel, dyadic_order, sym=False, _naive_solver=False, return_sol_grid=False):
        ...
        if X.device.type == 'cuda':
            ...
            #             pass
            G = torch.tensor(sig_kernel_Gram_varpar(G_static_.detach().numpy(), sym, _naive_solver),
                             dtype=G_static.dtype, device=G_static.device)
    ...
    def backward(ctx, grad_output):
        ...
        # if on CPU
            ...
            #             pass
            G_rev = torch.tensor(sig_kernel_Gram_varpar(G_static_rev.detach().numpy(), sym, _naive_solver),
                                 dtype=G_static.dtype, device=G_static.device)
...
class _SigKernelHigerOrderGram(torch.autograd.Function):
    ...
    def forward(ctx, K_XX, K_XY, K_YY, static_kernel, dyadic_order, lambda_, sym=False, _naive_solver=False):
        ...
        G_base = innerprodCKME(K_XX, K_XY, K_YY, lambda_, static_kernel,
                               sym=sym)  # <---- this is the only change compared to order 1

        G_base_ = G_base[:, :, 1:, 1:] + G_base[:, :, :-1, :-1] - G_base[:, :, 1:, :-1] - G_base[:, :, :-1, 1:]

        G_base_ = tile(tile(G_base_, 2, 2 ** dyadic_order) / float(2 ** dyadic_order), 3, 2 ** dyadic_order) / float(
            2 ** dyadic_order)

        start_pde_time = time.time()
        # if on GPU
        if K_XX.device.type == 'cuda':
            ...
            compute_sig_kernel_Gram_mat_varpar_from_increments_cuda[blockspergrid, threads_per_block](
                cuda.as_cuda_array(G_base_.detach()),
                MM + 1, NN + 1, n_anti_diagonals,
                cuda.as_cuda_array(G), _naive_solver)
            G = G[:, :, :-1, :-1]
            ...
            #             pass
            G = torch.tensor(sig_kernel_Gram_varpar(G_base_.detach().numpy(), sym, _naive_solver), dtype=G_base.dtype,
                             device=G_base.device)

        end_pde_time = time.time()

        # print(f'PDE TIME: {end_pde_time-start_pde_time}')
...
def innerprodCKME(K_XX, K_XY, K_YY, lambda_, static_kernel, sym=False):
    ...
    # H_X = torch.eye(m, device=K_XX.device, dtype=K_XX.dtype) - (1. / m) * torch.ones((m, m), device=K_XX.device,
    #                                                                                  dtype=K_XX.dtype)  # centering matrix
    # H_Y = torch.eye(n, device=K_YY.device, dtype=K_YY.dtype) - (1. / n) * torch.ones((n, n), device=K_YY.device,
    #                                                                                  dtype=K_YY.dtype)  # centering matrix
    ...
    # features.append([K_XX, K_XY, K_YY])
    # labels.append(G)
...
if __name__ == '__main__':
    ...
    static_kernel = RBFKernel(sigma=0.5)
    ...
    start_mmd = time.time()
    mmd_order1 = signature_kernel.compute_mmd(x, y, order=2)
    end_mmd = time.time()
    ...
    for k in range(len(features)):
        for i in range(n):
            for j in range(n):
                new_features.append([features[k][p][i][j] for p in range(len(features[0]))])
                new_labels.append(labels[k][i][j])

    for idx, tensor in enumerate(new_features):
        for i in range(len(new_features[0])):
            torch.save(tensor, f"MMD_Dataset/feature_tensor_{idx}_{i}.pt")


    for idx, tensor in enumerate(new_labels):
        torch.save(tensor, f"MMD_Dataset/label_tensor_{idx}.pt")

    # with open("mmd_fast_no_model_features_2", "wb") as fp:
    #     pickle.dump(new_features, fp)
    #
    # with open("mmd_fast_no_model_labels_2", "wb") as fp:
